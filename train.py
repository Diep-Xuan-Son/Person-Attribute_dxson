...
from utils.folder import Train_Dataset
...
def load_network(network):
    save_path = os.path.join('./checkpoints', 'net_14.pth')
...
parser.add_argument('--backbone', default='resnet152', type=str, help='backbone: resnet50, resnet34, resnet18, densenet121')
...
assert args.backbone in ['resnet152','resnet101', 'resnet50', 'resnet34', 'resnet18', 'densenet121']
...
data_dir = './doc1/data_duke_market_seg.json'
...
# ---------
image_datasets = {}
image_datasets['train'] = Train_Dataset(data_dir,  train_val='train')
image_datasets['val'] = Train_Dataset(data_dir, train_val='query')
...
# Model and Optimizer
# ------------------
model = get_model("mobilenet_v2", num_label, args.use_id, num_id=num_id)
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val':
                ...
                if(epoch_acc_last < epoch_acc):
                    epoch_acc_last = epoch_acc
                    save_network(model, epoch)
                draw_curve(epoch)
        time_epoch = time.time() - start_epoch
        print('Epoch complete in {:.0f}m {:.0f}s'.format(time_epoch // 60, time_epoch % 60))
